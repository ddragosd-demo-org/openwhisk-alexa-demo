 ...
 */var AlexaSDK=require('alexa-sdk'),handlers={LaunchRequest:function LaunchRequest(){this.emit(':ask','Welcome to Adobe Analytics.. Which report suite would you like to use?'+getReportSuites(),'You can choose from the following report suites'+getReportSuites())},ReportSuiteSelectionIntent:function ReportSuiteSelectionIntent(){this.emit(':ask','Ok. How can I help you?','I can currently tell you information about X, Y and Z')},OneshotReportIntent:function OneshotReportIntent(){this.emit(':tell','One shot report intent.. Metric is '+this.event.request.intent.slots.Metric.value+'. Duration is '+this.event.request.intent.slots.Duration.value)},ThankYouIntent:function ThankYouIntent(){this.emit(':tell','My pleasure, Goodbye')},Unhandled:function Unhandled(){this.emit(':tell','Unhandled')}};function getReportSuites(){return'Adobe I/O Portal, Bladerunner Lab'}function handleOneshotReportRequest(a,b,c){// Determine metric, using default if none provided
var e,f,d=getMetricFromIntent(a,!0);if(d.error)return e='Currently, I can tell you information about the following metrics: '+getAllMetricsText()+'Which metric would you like to load?',f='I\'m sorry, I don\'t have any data for that metric.',void c.ask(f,e);// Determine custom date
var g=getDurationFromIntent(a);// all slots filled, either from the user or by default values. Move to final request
getFinalReportResponse(a,d,g,c)}function main(a){return console.log('ALEXA Event',a.request.type+'!'),new Promise(function(b){var d=AlexaSDK.handler(a,{succeed:b});return d.registerHandlers(handlers),d.execute()})}exports.default=main;
